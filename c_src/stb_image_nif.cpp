...
static ERL_NIF_TERM to_file(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    if (argc != 6) {
        return erlang::nif::error(env, "expecting 3 arguments: filename and extension and type");
    }
    std::string filename, extension;
    ErlNifBinary result;
    int w = 0, h = 0, comp = 0;
    if (erlang::nif::get(env, argv[0], filename) && erlang::nif::get(env, argv[1], extension) && enif_inspect_binary(env, argv[2], &result),\
     erlang::nif::get(env, argv[3], &w), erlang::nif::get(env, argv[4], &h), erlang::nif::get(env, argv[5], &comp)) {
        if (extension == "png") {
            int stride_in_bytes = 0;
            int status = stbi_write_png(filename.c_str(), w, h, comp, result.data, stride_in_bytes);
            if (!status) {
                return erlang::nif::error(env, "Unsuccesful attempt to write to png");
            }
        } else if (extension == "bmp") {
            int status = stbi_write_bmp(filename.c_str(), w, h, comp, result.data);
            if (!status) {
                return erlang::nif::error(env, "Unsuccesful attempt to write to bmp");
            }
        } else if (extension == "tga") {
            int status = stbi_write_tga(filename.c_str(), w, h, comp, result.data);
            if (!status) {
                return erlang::nif::error(env, "Unsuccesful attempt to write to tga");
            }
            ...
            return erlang::nif::error(env, "Wrong extension.");
        }
    }
